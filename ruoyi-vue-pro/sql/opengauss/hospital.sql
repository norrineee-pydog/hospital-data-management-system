...
DROP VIEW IF EXISTS v_visit_detail;

CREATE VIEW v_visit_detail AS

SELECT
    v.id,
    v.patient_id,
    p.name as patient_name,           -- 患者姓名
    v.doctor_id,
    d.name as doctor_name,            -- 医生姓名
    v.dept_id,
    dept.dept_name as dept_name,      -- 科室名称
    v.visit_date,
    v.status,
    v.reason,
    v.diagnosis,
...
COMMENT ON VIEW v_visit_detail IS '就诊记录详情视图，关联患者、医生、科室名称';