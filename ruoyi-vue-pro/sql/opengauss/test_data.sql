-- =============================================
-- 医院数据管理系统 - 完整测试数据（含医生数据，适配实际表结构）
-- 执行方式: docker exec opengauss bash -c "export LD_LIBRARY_PATH=/usr/local/opengauss/lib; /usr/local/opengauss/bin/gsql -U gaussdb -W Hospital@2026 -d ruoyi_vue_pro -f /tmp/test_data_complete.sql"
-- =============================================

-- =============================================
-- 0. 系统用户 (system_users) - 医生和管理员账号
-- 注意：请根据你的实际system_users表结构调整字段
-- =============================================
...
(101, 'doctor_li', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EO', '李医生', '内科主任医师', 1, '[1]', 'doctor_li@hospital.com', '13800138101', 1, '', 0, '127.0.0.1', NOW(), '1', NOW(), '1', NOW(), 0, 1),
(102, 'doctor_wang', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EO', '王医生', '外科副主任医师', 3, '[2]', 'doctor_wang@hospital.com', '13800138102', 1, '', 0, '127.0.0.1', NOW(), '1', NOW(), '1', NOW(), 0, 1),
(103, 'admin_hospital', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EO', '系统管理员', '医院系统管理员', 1, '[1]', 'admin@hospital.com', '13800138100', 1, '', 0, '127.0.0.1', NOW(), '1', NOW(), '1', NOW(), 0, 1);

-- =============================================
-- 1. 科室 (hospital_department) - 已有2条(id=1,2)，补充3条
-- =============================================
...
-- =============================================
-- 2. 医生 (hospital_doctor) - 新增2条，适配实际表结构
-- =============================================
INSERT INTO hospital_doctor (id, user_id, dept_id, name, gender, age, title, license_no, phone, email, creator, create_time, updater, update_time, deleted, tenant_id) VALUES
(nextval('hospital_doctor_seq'), 101, 1, '李明华', '男', 45, '主任医师', '11010120150101001', '13800138101', 'doctor_li@hospital.com', '1', NOW(), '1', NOW(), 0, 1),
(nextval('hospital_doctor_seq'), 102, 3, '王建国', '男', 38, '副主任医师', '11010120180202002', '13800138102', 'doctor_wang@hospital.com', '1', NOW(), '1', NOW(), 0, 1);

-- =============================================
-- 3. 病房 (hospital_ward) - 新增6条
-- =============================================
...
-- =============================================
-- 5. 患者 (hospital_patient) - 已有1条(id=1)，补充4条
-- =============================================
...
-- =============================================
-- 6. 就诊记录 (hospital_visit) - 使用正确的医生ID (1或2)
-- =============================================
...
(3, 3, 2, 3, NOW() - INTERVAL '3 DAY', '腹部不适', '急性阑尾炎', '建议手术治疗', 'COMPLETED', '1', NOW(), '1', NOW(), 0, 1),
(4, 4, 1, 1, NOW() - INTERVAL '1 DAY', '高热39度', '上呼吸道感染', '物理降温+退热药，密切观察', 'COMPLETED', '1', NOW(), '1', NOW(), 0, 1),
(5, 5, 2, 3, NOW() - INTERVAL '6 HOUR', '胸闷气短', '待查（疑似心绞痛）', '心电图检查中', 'IN_PROGRESS', '1', NOW(), '1', NOW(), 0, 1);

-- =============================================
-- 7. 药品 (hospital_medicine) - 已有1条(id=1)，补充7条
-- =============================================
...
-- =============================================
-- 8. 处方 (hospital_prescription) - 对应就诊记录
-- =============================================
...
(3, 3, 2, 'DISPENSED', '术后抗感染处方', '1', NOW(), '1', NOW(), 0, 1),
(4, 4, 1, 'DISPENSED', '退热消炎处方', '1', NOW(), '1', NOW(), 0, 1),
(5, 5, 2, 'PENDING', '心绞痛急救处方（待发药）', '1', NOW(), '1', NOW(), 0, 1);

-- =============================================
-- 9. 处方明细 (hospital_prescription_item)
-- =============================================
...
(3, 3, 3, 53.90, 53.90, NOW() - INTERVAL '3 DAY', 'CARD', 'PAID', '1', NOW(), '1', NOW(), 0, 1),
